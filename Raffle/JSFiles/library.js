function drawARaffle(t) {
    $(".winning p").removeClass("pitch"), $(t.button).unbind("click").css("cursor", "default"), Math.ceil(1e3 * Math.random()) + 100, slotMachine(t, 1, list)
    ...
    t.list.children("p").removeClass("pitch"), $(t.button).unbind("click").css("cursor", "default"); var s = Math.ceil(200 * Math.random()) + 90; slotMachine(t.list, s), setTimeout(function () {
        $(t.list.children("p")[297 - s]).addClass("pitch"); for (var i = $(t.list.children("p")[297 - s]).html(), o = $(".pitch").html(), a = n.length, c = 0; a > c; c++) o == n[c]; setTimeout(function () {
            $(".screen span").html(i), $(".screen").css("display", "flex"), $(".prize").show("nomal"), $(t.button).on("click", function () {
                t.list.remove(), t.list = $("<div>", { "class": "list" }).appendTo(t.winning), t.joysticks.addClass("joysticksAnimate"), setTimeout(function () {
                    t.joysticks.removeClass("joysticksAnimate")
                }, 1850), putList(t.list, n), animateButton($(t.button)), drawARaffle(t, n)
            }).css("cursor", "pointer")
        }, 400)
    }, 15500)
} var list;